...
#define F (1 << 14) //for MLFQS

/*  of processes in THREAD_READY state, that is, processes
...
void threadGetPriority (struct thread *t, void *aux UNUSED);//sets priority
void getRecentCPU(struct thread *t,void *aux UNUSED);//sets the recent cpu
...
void getRecentCPU(struct thread *t,void *aux UNUSED){
	...
	t->recent_cpu = addFPandINT(multiplyFixed(divFixed(speedup,addFPandINT(speedup,1)),t->recent_cpu),t->nice);
}
...
thread_unblock (struct thread *t) 
{
  ...
  ASSERT (t->status == THREAD_BLOCKED);
	list_insert_ordered(&ready_list, &t->elem,(list_less_func *) &compare_priority, NULL);		//highest priority should be at front
  t->status = THREAD_READY;
  intr_set_level (old_level);
}
bool compare_priority (const struct list_elem *first,
                                const struct list_elem *second,
                                void *aux UNUSED) //compare function to make higher priority functions first
{
...
thread_yield (void) 
{
  ...
  old_level = intr_disable ();
  if (cur != idle_thread) 
    list_insert_ordered(&ready_list, &cur->elem,(list_less_func *) &compare_priority, NULL);		//highest priority should be at front
...
thread_set_priority (int new_priority) 
{
	enum intr_level old_level;
	old_level = intr_disable ();
	//if thread is being donated a priority, check if the priority should continue to be donated
	if(!list_empty(&thread_current()->donations)&&new_priority < thread_current()->priority){
	...
	//if no priority donation, change priority and check if thread needs to be preempted
  thread_current()->priority = new_priority;
  thread_current()->initial_priority = new_priority;
  sort_ready();
  struct list_elem *thread_elem = list_begin(&ready_list);
	struct thread *s = list_entry(thread_elem, struct thread, elem);
  if(thread_current()->priority < s->priority){
		thread_yield();
	}
	intr_set_level (old_level);
	/*
  while(true){
  	...
  	struct list_elem *thread_elem = list_next(thread_elem);
  	struct thread *s = list_entry(thread_elem,
				struct thread, elem);
  	if(is_tail(thread_elem)){
  		break;
  	}
  }*/
}
...
{
  /* Not yet implemented. */
  return fixedPointToIntRound(multiplyFPandINT(load_average, 100));
}
...
{
  /* Not yet implemented. */
  return fixedPointToIntRound(multiplyFPandINT(thread_current()->recent_cpu, 100));
}

